...
description: "Use when the user wants the Claude Code Game Studios \"brainstorm\" workflow in Codex. Guided game concept ideation - from zero idea to a structured game concept document. Uses professional studio ideation techniques, player psychology frameworks, and structured creative exploration."
---

# CCGS Brainstorm

Codex adaptation of the Claude Code Game Studios `brainstorm` workflow.

## Codex Translation Layer

- Original slash command: `/brainstorm`
- Codex skill name: `ccgs-brainstorm`
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `ask the user directly with a concise plain-text question`** at key decision points throughout brainstorming:
   ...
   `ask the user directly with a concise plain-text question` to capture the decision with concise labels.
...
6. **Suggest next steps** (in this order - this is the professional studio
   ...
   - "Run `ccgs-setup-engine [engine] [version]` to configure the engine and populate version-aware reference docs"
   - "Use `ccgs-design-review design/gdd/game-concept.md` to validate completeness"
   - "Discuss vision with the `creative-director` agent for pillar refinement"
   - "Decompose the concept into individual systems with `ccgs-map-systems` - maps dependencies, assigns priorities, and creates the systems index"
   - "Author per-system GDDs with `ccgs-design-system` - guided, section-by-section GDD writing"
   - "Prototype the core loop with `ccgs-prototype [core-mechanic]`"
   - "Playtest the prototype with `ccgs-playtest-report` to validate the hypothesis"
   - "If validated, plan the first sprint with `ccgs-sprint-plan new`"
