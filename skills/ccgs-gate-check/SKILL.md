...
description: "Use when the user wants the Claude Code Game Studios \"gate-check\" workflow in Codex. Validate readiness to advance between development phases. Produces a PASS/CONCERNS/FAIL verdict with specific blockers and required artifacts."
---

# CCGS Gate Check

Codex adaptation of the Claude Code Game Studios `gate-check` workflow.

## Codex Translation Layer

- Original slash command: `/gate-check`
- Codex skill name: `ccgs-gate-check`
...
- **No argument**: Auto-detect current stage using the same heuristics as
  `ccgs-project-stage-detect`, then validate the NEXT phase transition
...
- [ ] At least 1 playtest report (or `ccgs-playtest-report` has been run)
...
- **Missing ADRs?** -> `ccgs-architecture-decision`
