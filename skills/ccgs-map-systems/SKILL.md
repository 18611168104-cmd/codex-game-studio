...
description: "Use when the user wants the Claude Code Game Studios \"map-systems\" workflow in Codex. Decompose a game concept into individual systems, map dependencies, prioritize design order, and create the systems index."
---

# CCGS Map Systems

Codex adaptation of the Claude Code Game Studios `map-systems` workflow.

## Codex Translation Layer

- Original slash command: `/map-systems`
- Codex skill name: `ccgs-map-systems`
...
- Use `ask the user directly with a concise plain-text question` to ask:
...
Then use `ask the user directly with a concise plain-text question` to capture feedback:
...
Use `ask the user directly with a concise plain-text question` to ask: "Does this dependency ordering look right? Any
...
Use `ask the user directly with a concise plain-text question` to ask: "Do these priority assignments match your vision?
...
## 7. Phase 6: Design Individual Systems (Handoff to ccgs-design-system)
...
Use `ask the user directly with a concise plain-text question` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `ccgs-design-system` completes, use `ask the user directly with a concise plain-text question`:
...
- "Run `ccgs-design-system [system-name]` to write the next system's GDD"
- "Run `ccgs-design-review [path]` on each completed GDD to validate quality"
- "Run `ccgs-gate-check pre-production` to check if you're ready to start building"
- "Prototype the highest-risk system with `ccgs-prototype [system]`"
- "Plan the first implementation sprint with `ccgs-sprint-plan new`"
...
2. **ask the user directly with a concise plain-text question** at every decision point (Explain -> Capture pattern):
