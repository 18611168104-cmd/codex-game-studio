---
name: ccgs-start
description: "Use when the user wants the Claude Code Game Studios \"start\" workflow in Codex. First-time onboarding - asks where you are, then guides you to the right workflow. No assumptions."
---

# CCGS Start

Codex adaptation of the Claude Code Game Studios `start` workflow.

## Codex Translation Layer

- Original slash command: `/start`
- Codex skill name: `ccgs-start`
...
2. Briefly explain what `ccgs-brainstorm` does (guided ideation using professional
   frameworks - MDA, player psychology, verb-first design)
3. Recommend running `ccgs-brainstorm open` as the next step
4. Show the recommended path:
   - `ccgs-brainstorm` - discover your game concept
   - `ccgs-setup-engine` - configure the engine (brainstorm will recommend one)
   - `ccgs-map-systems` - decompose the concept into systems and plan GDD writing order
   - `ccgs-prototype` - test the core mechanic
...
2. Based on their answers, offer two paths:
   - **Formalize first**: Run `ccgs-brainstorm` to structure the concept into a
   ...
   - **Jump to engine setup**: If they're confident in their concept, go
     straight to `ccgs-setup-engine` and write the GDD manually afterward
3. Show the recommended path (adapted to their choice):
   - `ccgs-brainstorm` or `ccgs-setup-engine` (their pick)
   - `ccgs-design-review` - validate the concept doc
   - `ccgs-map-systems` - decompose the concept into individual systems with dependencies and priorities
   - `ccgs-design-system` - author per-system GDDs (guided, section-by-section)
   - `ccgs-architecture-decision` - make first technical decisions
   - `ccgs-sprint-plan` - plan the first sprint
...
2. Recommend running `ccgs-project-stage-detect` for a full analysis
3. If the engine isn't configured, note that `ccgs-setup-engine` should come first
4. Show the recommended path:
   - `ccgs-project-stage-detect` - full gap analysis
   - `ccgs-setup-engine` - if not configured
   - `ccgs-design-system` - if systems index exists but GDDs are incomplete
   - `ccgs-gate-check` - validate readiness for next phase
   - `ccgs-sprint-plan` - organize the work
...
or offer to run it for them. Either way, the `ccgs-start` skill's job is done
...
- **User is returning (engine configured, concept exists)**: Skip onboarding
  ...
  you left off? Try `ccgs-sprint-plan` or just tell me what you'd like to work on."
