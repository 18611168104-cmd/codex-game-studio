...
> "Usage: `ccgs-design-system <system-name>` - e.g., `ccgs-design-system combat-system`
> Run `ccgs-map-systems` first to create the systems index, then use this skill
...
Use `ask the user directly with a concise plain-text question`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask the user directly with a concise plain-text question` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask the user directly with a concise plain-text question` to capture the decision.
...
Use `ask the user directly with a concise plain-text question`:
- "Run `ccgs-design-review` now to validate the GDD?"
...
- The main session presents the agent's output to the user via `ask the user directly with a concise plain-text question`
