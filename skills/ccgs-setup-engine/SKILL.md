...
description: "Use when the user wants the Claude Code Game Studios \"setup-engine\" workflow in Codex. Configure the project's game engine and version. Pins the engine in CLAUDE.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyond the LLM's training data."
---

# CCGS Setup Engine

Codex adaptation of the Claude Code Game Studios `setup-engine` workflow.

## Codex Translation Layer

- Original slash command: `/setup-engine`
- Codex skill name: `ccgs-setup-engine`
...
- If no version provided, use web browsing to find the latest stable release:
...
`C:\Users\18143\.codex\skills\ccgs-studio\references\repo\docs\engine-reference\<engine>\VERSION.md`
```
...
4. Use web browsing to verify uncertain APIs
...
2. Use web browsing to check for:
...
4. [If from ccgs-brainstorm] Run ccgs-prototype [core-mechanic] to test the core loop
...
- NEVER guess an engine version - always verify via web browsing or user confirmation
...
- If web browsing returns ambiguous results, show the user and let them decide
